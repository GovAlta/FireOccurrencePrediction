...
LTG_STRIKE_CSV_HEADERS_2 = ['LOCAL_STRIKETIME', 'LATITUDE', 'LONGITUDE', 'STRENGTH', 'MULTIPLICITY']
...
def rawWeatherDataMassager(input_df, output_path, weather_station_locations_path):
        ...
        input_df['rain_mm'] = input_df['precipitation'].apply(lambda x : round(x, 2))
        input_df['rain_mm'] = input_df['precipitation'].apply(lambda x : -999.9 if isnan(x) else x)

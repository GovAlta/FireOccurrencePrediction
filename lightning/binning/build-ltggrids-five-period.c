...
int main(int argc, char *argv[]){
  ...
  // build-ltggrids-five-period.exe "Y:\\University of Alberta\\Software Development\\fire-occurrence-predictor-repo\\lightning\\binning\\Gridlocations.prn" "Z:\\ LightningFireOccurrencePredictionInputs\\ABltg_space_MATT.out" "Z:\\ LightningFireOccurrencePredictionInputs\\ltg2010-20by20-five-period.dat"
  ...
  err=fscanf(inp,"%f%f%f%d%d%d%d%d",&lat,&lon,&stren,&mult,&year,&mon,&day,&hour);
     printf("lat=%f   lon=%f  ms%f m=%d y%d mon%d day%d hh%d \n",lat,lon, stren,mult,  year,mon,day, hour);
  jd=julian(mon,day);
